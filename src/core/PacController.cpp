...
PacController::PacController(Logger& logger, NTCReader& sondes, Consignes& consignes, Temporisations& temporisations)
    : m_logger(logger),
      m_sondes(sondes),
...
ModePAC PacController::determineMode(double tempExt) const {
    ...
    if (tempExt < consCanicule - hystEteHiver) return MODE_FROID;
    return MODE_ARRET;
}
...
void PacController::loop() {
    ...
    while (!m_stopFlag.load()) {
        ...
        {
            if (!m_securite) {
                m_logger.error("[SECURITE] Sonde défectueuse détectée, arrêt immédiat de la PAC !");
                m_securite = true;
            ...
            m_logger.info("[SECURITE] Toutes les sondes OK, reprise de la PAC.");
            m_securite = false;
        ...
        if (std::chrono::duration_cast<std::chrono::seconds>(now - lastLog).count() >= 1) {
            ...
            pac_ss << "[PAC] Mode=" << modeToString(m_modeActuel);
            m_logger.info(pac_ss.str());
