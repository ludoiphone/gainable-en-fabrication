      ...
      currentIndex(-1)
{
    ...
    m_background->setPixmap(QPixmap("/home/ludo/Desktop/test4/images/modifCons.jpg"));
    m_background->setGeometry(0, 0, 1024, 600);
    m_background->lower();

    // --- Initialisation des consignes ---
    m_modifConsignes.reset(new Consignes("consignes.ini"));
    ...
    // --- 14 consignes cohérentes ---
    addConfig("Eté / Hiver", "ConsigneEteHiver", 10.0, 16.0, 13.5);
    addConfig("Canicule", "ConsigneCanicule", 28.0, 32.0, 30.0);
    addConfig("Delta", "ConsigneDelta", -10.0, 0.0, -6.0);
    addConfig("Blocage Chauffage", "ConsigneBlocageChauffage", 9.0, 13.0, 11.0);
    addConfig("GV Ext Chauffage", "ConsigneGrandeVitesseExterieurChauffage", -5.0, 1.0, -2.0);
    addConfig("GV Ext Froid", "ConsigneGrandeVitesseExterieurFroid", 16.0, 22.0, 20.0);
    addConfig("Fin Dégivrage Chauffage", "ConsigneFinDegivrageChauffage", 10.0, 14.0, 12.5);
    addConfig("PV Int Chauffage", "ConsignePetiteVitesseInterieurChauffage", 21.0, 25.0, 23.0);
    addConfig("GV Int Froid", "ConsigneGrandeVitesseInterieurFroid", 21.0, 25.0, 23.0);
    addConfig("Départ Ventil Int Chauffage", "ConsigneDepartVentilateurInterieurChauffage", 30.0, 40.0, 35.0);
    addConfig("Dégivrage Froid", "ConsigneDegivrageFroid", -5.0, -1.0, -3.0);
    addConfig("Fin Dégivrage Froid", "ConsigneFinDegivrageFroid", 13.0, 17.0, 15.0);
    addConfig("Ventil Int Canicule", "ConsigneVentilationInterieurCanicule", 18.0, 25.0, 20.0);
    addConfig("PV Ext Froid", "ConsignePetiteVitesseExterieurFroid", 14.0, 18.0, 16.0);

    createConsigneButtons();

    // --- Zone de modification ---
    m_groupBox = new QGroupBox(this);
    m_groupBox->setGeometry(579, 200, 419, 200);
    m_groupBox->setFont(QFont("Times", 12, QFont::Bold));
    m_groupBox->setStyleSheet("QGroupBox::title { color: red; }");
    m_groupBox->hide();
    ...
    m_lastValueLabel->setGeometry(10, 10, 399, 30);
    ...
    m_selCons ->setStyleSheet("QDoubleSpinBox::down-button{width: 70} QDoubleSpinBox::up-button{width: 70}");
    m_selCons->hide();
...
void ModifCons::createConsigneButtons()
{
    for (int i = 0; i < configs.size(); ++i) {
        ...
        QString color = configs[i].titre.contains("Froid") ? "blue"
                        : configs[i].titre.contains("Canicule") ? "orange"
...
void ModifCons::saveCons()
{
    ...
    qDebug() << "Consigne sauvegardée :" << configs[currentIndex].titre << "=" << val;
